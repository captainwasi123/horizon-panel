...
class bahriaLeadsController extends Controller
{
    ...
    function index(){
       if (Auth::check()) {
            ...
            return view('bahriaLeads.list', ['databelt' => $databelt, 'status' => $status, 'source' => $source]);
        }else{
    ...
    function totalQueries(){
       if (Auth::check()) {
            ...
            return view('bahriaLeads.totalQueries', ['databelt' => $databelt, 'status' => $status, 'category' => $category, 'users' => $user, 'filter' => '0']);
        }else{
            return redirect('/login')->with('error', 'Authentication Error');
        } 
    }
    ...
    function totalCategory($id, $name){
       if (Auth::check()) {
            ...
            $databelt = lead::where('trash', null)
                        ...
                        ->where('cat_id', $id)
                        ->orderBy('created_at', 'desc')->get();
            return view('bahriaLeads.totalQueriesCategory', ['databelt' => $databelt, 'status' => $status, 'category' => $category, 'users' => $user, 'filter' => '0', 'name' => $name]);
        }else{
            return redirect('/login')->with('error', 'Authentication Error');
        } 
    }
    ...
    function totalQueriesSubmit(Request $request){
       if (Auth::check()) {
            ...
            $databelt = lead::where('trash', null)
                        ...
                        ->where('lead_status', '1')
                        ->orderBy('created_at', 'desc')->get();
            return view('bahriaLeads.totalQueries', ['databelt' => $databelt, 'status' => $status, 'category' => $category, 'users' => $user, 'filter' => '1', 'search_data' => $data]);
        }else{
            return redirect('/login')->with('error', 'Authentication Error');
        } 
    }
    ...
    function superpotentialQueriesSubmit(Request $request){
       if (Auth::check()) {
            ...
            $databelt = lead::where('trash', null)
                        ...
                        ->where('lead_status', '2')
                        ->orderBy('created_at', 'desc')->get();
            return view('bahriaLeads.superPotentialQueries', ['databelt' => $databelt, 'status' => $status, 'category' => $category, 'users' => $user, 'filter' => '1', 'search_data' => $data]);
        }else{
            return redirect('/login')->with('error', 'Authentication Error');
        } 
    }
    ...
    function pendingLeads(){
       if (Auth::check()) {
            ...
            $databelt = lead::where('trash', null)
                        ...
                        ->whereIn('cat_id', ['1', '2', '3'])
                        ->orderBy('created_at', 'desc')->get();
            return view('bahriaLeads.pendingLead', ['databelt' => $databelt, 'status' => $status, 'category' => $category, 'users' => $user, 'filter' => '0']);
        }else{
            return redirect('/login')->with('error', 'Authentication Error');
        } 
    }
    ...
    function pendingLeadsSubmit(Request $request){
       if (Auth::check()) {
            ...
            $databelt = lead::where('trash', null)
                        ...
                        ->where('lead_status', '6')
                        ->orderBy('created_at', 'desc')->get();
            return view('bahriaLeads.pendingLead', ['databelt' => $databelt, 'status' => $status, 'category' => $category, 'users' => $user, 'filter' => '1', 'search_data' => $data]);
        }else{
            return redirect('/login')->with('error', 'Authentication Error');
        } 
    }
    ...
    function addLead(){
       if (Auth::check()) {
            ...
            $source = source::all();
            $category = category::all();
            return view('bahriaLeads.new', ['status' => $status, 'source' => $source, 'category' => $category]);
        }else{
            return redirect('/login')->with('error', 'Authentication Error');
        } 
    }
    ...
    function editLead($id){
       if (Auth::check()) {
            ...
            return view('bahriaLeads.edit', ['data' => $data, 'status' => $status, 'source' => $source, 'category' => $category]);
        }else{
            return redirect('/login')->with('error', 'Authentication Error');
        } 
    }
    ...
    function loadRemarks($id){
       if (Auth::check()) {
            $data = Lead::find($id);
            return view('bahriaLeads.remarksResponse', ['data' => $data]);
        }else{
            return redirect('/login')->with('error', 'Authentication Error');
        } 
    }
